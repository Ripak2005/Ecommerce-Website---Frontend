...
const AdminOrders = () => {
  ...
  const updateStatus = async(id) => {
    if(confirm("Are you sure wnat to update status of this order")){
      setLoading(true)
      try {
      const {data} = await axios.put(`${server}/api/order/${id}`, {},  {
        headers: {
          token: localStorage.getItem("token"),
        }
      })
              ...
              <tr>
                ...
                <td>
                  {e.status === "Delivered" ? (
                    ...
                    <Button disabled={loading} onClick={() => updateStatus(e._id)}>
                      Update Status
                    </Button>
                  )}
                </td>
